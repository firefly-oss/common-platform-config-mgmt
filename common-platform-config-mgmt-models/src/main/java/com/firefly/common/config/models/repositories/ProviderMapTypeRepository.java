package com.firefly.common.config.models.repositories;

import com.firefly.common.config.models.entities.ProviderMapType;
import org.springframework.stereotype.Repository;
import reactor.core.publisher.Flux;
import reactor.core.publisher.Mono;
import java.util.UUID;

@Repository
public interface ProviderMapTypeRepository extends BaseRepository<ProviderMapType, UUID> {

    Flux<ProviderMapType> findByActiveTrue();

    Mono<ProviderMapType> findByCode(String code);

    Flux<ProviderMapType> findByProviderMappingStatusId(UUID providerMappingStatusId);
}
